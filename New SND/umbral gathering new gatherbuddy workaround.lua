...
function Gather()
 ...
 --[[     if IsAddonVisible("Gathering") and IsAddonReady("Gathering") then
    ...
    --temp replacement code
    -- Robust replacement using Addons.GetAddon("Gathering").Ready
    do
        local ok, addon = pcall(function() return Addons and Addons.GetAddon and Addons.GetAddon("Gathering") end)
        if not ok then
            if DEBUG_FULL then Dalamud.Log("[UmbralGathering] probe: Addons.GetAddon call errored") end
            -- fall back to skipping the gathering-callback behaviour
            return
        end

        if not addon then
            if DEBUG_FULL then Dalamud.Log("[UmbralGathering] probe: Addon 'Gathering' not found (nil)") end
            return
        end

        -- If the addon reports Ready=true, treat that as the start of the wait-for-close behavior
        if addon.Ready then
            if DEBUG_FULL then Dalamud.Log("[UmbralGathering] Gathering addon Ready=true → entering wait-for-close loop") end

            local falseCount = 0
            while falseCount < 2 do
                -- wait 1 second between checks (matches your requirement)
                yield("/wait 1")

                local ok2, addon2 = pcall(function() return Addons and Addons.GetAddon and Addons.GetAddon("Gathering") end)
                if not ok2 or not addon2 or not addon2.Ready then
                    falseCount = falseCount + 1
                    if DEBUG_FULL then Dalamud.Log(string.format(
                        "[UmbralGathering] Gathering Ready=false (consecutive false count=%d)", falseCount)) end
                else
                    -- addon still ready; reset false counter and keep waiting
                    falseCount = 0
                    if DEBUG_FULL then Dalamud.Log("[UmbralGathering] Gathering Ready still true; resetting falseCount") end
                end
            end

            -- seen false twice in a row — wait additional 4s before continuing (as requested)
            yield("/wait 4")
            if DEBUG_FULL then Dalamud.Log("[UmbralGathering] Gathering Ready observed false x2 → exiting wait section") end

            -- exit the section the same way the original block would (i.e. don't attempt callback)
            return
        end

        -- If we reach here, addon exists but addon.Ready ~= true (not ready).
        -- The original block only ran when addon was visible+ready, so we just do nothing.
        if DEBUG_FULL then Dalamud.Log("[UmbralGathering] Gathering addon not ready (no action)") end
    end
end
